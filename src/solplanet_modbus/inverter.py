...
class PvInput(SolplanetComponent):
    ...
    voltage = gauge(1318, 0.1, signed=False, unit="V")
    current = gauge(1319, 0.01, signed=False, unit="A")


class PvString(SolplanetComponent):
    """One monitored PV string of an inverter. Read with :class:`InverterData`."""

    current = gauge(1338, 0.1, signed=False, unit="A")
...
class InverterInfo(SolplanetComponent):
    ...
    modbus_address = integer(1001, signed=False)
    ...
    grid_code = integer(1026, signed=False)
    ...
    brand = string(1064, 8)
    model = enum(1072, InverterModel)
    mppt_count = integer(1073, signed=False)
    string_current_count = integer(1074, signed=False)
    ...
    string_count_pv1_4 = integer(1116, signed=False)
    string_count_pv5_8 = integer(1117, signed=False)
    string_count_pv9_12 = integer(1118, signed=False)
    string_count_pv13_16 = integer(1119, signed=False)
    string_count_pv17_20 = integer(1120, signed=False)
    string_count_pv21_24 = integer(1121, signed=False)
    string_count_pv25_28 = integer(1122, signed=False)
    string_count_pv29_32 = integer(1123, signed=False)
...
class InverterData(SolplanetComponent):
    ...
    Values UM0058 marks optional read zero on an inverter without the hardware —
    the extra PV inputs, the per-string currents, the L2/L3 and line voltages of
    a single-phase inverter, and the DC/DC converter temperature of a non-hybrid.
    """

    register_ranges = ((1300, 1388),)

    grid_rated_voltage = gauge(1300, 0.1, signed=False, unit="V")
    grid_rated_frequency = gauge(1301, 0.01, signed=False, unit="Hz")
    energy_today = uint32(1302, scale=0.1, unit="kWh")
    energy_total = uint32(1304, scale=0.1, unit="kWh")
    hours_total = uint32(1306, unit="h")
    state = enum(1308, DeviceState)
    connect_time = integer(1309, signed=False, unit="s")

    internal_temperature = gauge(1310, 0.1, unit="°C")
    phase_u_temperature = gauge(1311, 0.1, unit="°C")
    phase_v_temperature = gauge(1312, 0.1, unit="°C")
    phase_w_temperature = gauge(1313, 0.1, unit="°C")
    boost_temperature = gauge(1314, 0.1, unit="°C")
    dc_converter_temperature = gauge(1315, 0.1, unit="°C")
    bus_voltage = gauge(1316, 0.1, signed=False, unit="V")

    pv_inputs = repeating_group(PV_INPUT_COUNT, PvInput, stride=2)
    strings = repeating_group(STRING_COUNT, PvString, stride=1)

    l1_voltage = gauge(1358, 0.1, signed=False, unit="V")
    l1_current = gauge(1359, 0.1, signed=False, unit="A")
    l2_voltage = gauge(1360, 0.1, signed=False, unit="V")
    l2_current = gauge(1361, 0.1, signed=False, unit="A")
    l3_voltage = gauge(1362, 0.1, signed=False, unit="V")
    l3_current = gauge(1363, 0.1, signed=False, unit="A")
    rs_line_voltage = gauge(1364, 0.1, signed=False, unit="V")
    rt_line_voltage = gauge(1365, 0.1, signed=False, unit="V")
    st_line_voltage = gauge(1366, 0.1, signed=False, unit="V")
    grid_frequency = gauge(1367, 0.01, signed=False, unit="Hz")

    apparent_power = uint32(1368, unit="VA")
    active_power = int32(1370, unit="W")
    reactive_power = int32(1372, unit="var")
    power_factor = gauge(1374, 0.01)

    fault_state = enum(1376, FaultState)
    # UM0058 defers both code tables to a "section 3.4" the released manual does
    # not contain, so the raw values are all this library can offer.
    error_code = integer(1377, signed=False)
    warning_code = integer(1378, signed=False)
    ios_measure = integer(1388, signed=False)
